...
int main() {
	int X, Y;
	unsigned char* originalImage = ReadBmp("60.bmp", &X, &Y);
	...
	for (int theta = -90; theta < 90; theta ++){
		Rotate(outputImage, originalImage, X, Y, theta);
		// Check White row
		currentWhite = 0;
		for (int iy = 0; iy < Y; iy++) {
			sum = 0;
			for (int ix = 0; ix < X; ix++) {
				sum += (int)(outputImage[(iy * X + ix) * 3] 
					+ outputImage[(iy * X + ix) * 3 + 1] 
					+ outputImage[(iy * X + ix) * 3 + 2]) / 3;
			}
			avg = sum / X;
			if (avg > 250)
				currentWhite ++;
	...
	Rotate(outputImage, originalImage, X, Y, optimalAngle);
	cout << "Rotated " << optimalAngle << " degree" << endl;
	WriteBmp("60_transformed.bmp", outputImage, X, Y);
